...
export function ShortcutModal({ bottleId, initial, onClose }: Props) {
  ...
  const submit = async () => {
    const rt = (runtime || null) as Shortcut['runtime']
    try {
      if (initial.id) {
        await ipc.updateShortcut(bottleId, { id: initial.id, name, exePath: initial.exePath, args, runtime: rt })
      } else {
        await ipc.addShortcut(bottleId, { name, exePath: initial.exePath, args, runtime: rt })
      }
      await load()
      onClose()
    } catch (e) {
  ...
  return (
    <Modal title={initial.id ? t.editShortcut : t.newShortcut} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          <label className="mb-1 block text-sm text-zinc-400">{t.shortcutEngineLabel}</label>
          <select
            className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-zinc-100"
            value={runtime}
            onChange={(e) => setRuntime(e.target.value)}
          >
            <option value="">{t.engineFollowBottle}</option>
            <option value="stable">{t.runtimeStable}</option>
            <option value="staging">{t.runtimeStaging}</option>
            <option value="crossover">{t.runtimeCrossover}</option>
          </select>
          <p className="mt-1 text-xs text-zinc-600">{t.shortcutEngineHint}</p>
        </div>
